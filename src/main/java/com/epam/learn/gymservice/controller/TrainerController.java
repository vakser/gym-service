...
public class TrainerController {
    ...
    public ResponseEntity<String> addTraining(@PathVariable String username,
        ...
        String token = authenticationFacade.extractAuthToken(headers);
        if (!token.isEmpty() && jwtTokenUtil.getUsernameFromToken(token).equals(username)) {
            if (jwtTokenUtil.getUsernameFromToken(token).equals(addTrainingRequest.getTrainerUsername())) {
                trainerService.addTraining(addTrainingRequest);
                return ResponseEntity.ok().build();
            } else {
                return new ResponseEntity<>("You cannot add new training for another trainer", HttpStatus.FORBIDDEN);
            }
        } else {
            return new ResponseEntity<>(HttpStatus.FORBIDDEN);
        }
